class BaseMeta(type):
    def __new__(mcs, name, parents, dct):
        ...
        attributes = {k: v for k, v in dct.items() if not k.startswith('_') and k != 'pk'}
        ...
        if '__repr__' not in dct:
            def repr_by_pk(self):
                if isinstance(self.pk, (list, tuple)):
                    attr_repr = ', '.join(['{}'] * len(self.pk)).format(*[getattr(self, i) for i in self.pk])
                else:
        ...
        if '__hash__' not in dct:
            dct['__hash__'] = lambda self: hash(tuple([getattr(self, i) for i in self.pk]))
        if '__eq__' not in dct:
            dct['__eq__'] = lambda self, other: isinstance(other, self.__class__) and hash(self) == hash(other)
            ...
            if getter_name not in dct:
                dct[getter_name] = \
                    (lambda k: classmethod(
                        lambda cls, data, default=None: data.get(cls.attributes[k], default.get(k, None))))(k)

        # Method to create instances from dict data
        def from_dict(cls, data, **default):
            model_data = {k: getattr(cls, '_get_{}_from_dict'.format(k))(data, default)
